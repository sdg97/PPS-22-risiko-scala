...
class GameSettingManagerTest extends AnyFunSpec with Matchers:
  describe("Game Settings Manager"){
    ...
    it("should have a way to verify if there is one or more username empty"){
      assert(manager.setGameSettings(List(("Player1","RED"),("","YELLOW"),("Player3","GREEN")),"Classic").equals(MessageSetting.ErrorIncompleteUsernames))
    }
    it("should have a way to verify if there is one or more username duplicated") {
      assert(manager.setGameSettings(List(("Player1","RED"),("Player1","YELLOW"),("Player3","GREEN")),"Classic").equals(MessageSetting.ErrorDuplicateUsername))
    }
    it("should have a way to verify if a color was assigned to multiple users") {
      assert(manager.setGameSettings(List(("Player1","RED"),("Player2","RED"),("Player3","GREEN")),"Classic").equals(MessageSetting.ErrorSameColorsSelected))
    }
    it("should have a way to verify if the version of map was not selected"){
      assert(manager.setGameSettings(List(("Player1","RED"),("Player2","YELLOW"),("Player3","GREEN")),"").equals(MessageSetting.ErrorVersionOfMap))
    }
    it("should have a way to set the initial settings of game"){
      assert(manager.setGameSettings(List(("Player1","RED"),("Player2","YELLOW"),("Player3","GREEN")),"Classic").equals(MessageSetting.CorrectSettings))
    }
